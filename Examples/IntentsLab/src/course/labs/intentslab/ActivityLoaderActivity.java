...
public class ActivityLoaderActivity extends Activity {
	...
	private void startExplicitActivation() {
		...
		Intent intentExp = new Intent(ActivityLoaderActivity.this, ExplicitlyLoadedActivity.class);
		
		//  - Start an Activity using that intent and the request code defined above
		startActivityForResult(intentExp, GET_TEXT_REQUEST_CODE);
	...
	private void startImplicitActivation() {
		...
		Intent intentImp = new Intent(Intent.ACTION_VIEW, Uri.parse(URL));
		...
		Intent chooserIntent = Intent.createChooser(intentImp, CHOOSER_TEXT);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode == GET_TEXT_REQUEST_CODE && resultCode == Activity.RESULT_OK){
			mUserTextView.setText(data.getStringExtra("inputText"));
		}
